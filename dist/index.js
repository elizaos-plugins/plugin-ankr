...
async function validateankrConfig(runtime) {
  try {
    const envConfig = getConfig(runtime.getSetting("ankr_ENV") ?? void 0);
    const config14 = {
      ANKR_ENV:
        process.env.ANKR_ENV ||
        runtime.getSetting("ANKR_ENV") ||
        envConfig.ANKR_ENV,
      ANKR_API_KEY:
      ...
      ANKR_MAX_RETRIES:
        process.env.ANKR_MAX_RETRIES ||
        runtime.getSetting("ANKR_MAX_RETRIES") ||
        envConfig.ANKR_MAX_RETRIES.toString(),
      ANKR_RETRY_DELAY:
        process.env.ANKR_RETRY_DELAY ||
        runtime.getSetting("ANKR_RETRY_DELAY") ||
        envConfig.ANKR_RETRY_DELAY.toString(),
      ANKR_TIMEOUT:
        process.env.ANKR_TIMEOUT ||
        runtime.getSetting("ANKR_TIMEOUT") ||
        envConfig.ANKR_TIMEOUT.toString(),
      ANKR_GRANULAR_LOG: process.env.ANKR_GRANULAR_LOG === "true" || false,
      ANKR_LOG_LEVEL:
        process.env.ANKR_LOG_LEVEL ||
        runtime.getSetting("ANKR_LOG_LEVEL") ||
        envConfig.ANKR_LOG_LEVEL,
      ANKR_RUNTIME_CHECK_MODE:
        process.env.RUNTIME_CHECK_MODE === "true" || false,
      ANKR_SPASH: process.env.ANKR_SPASH === "true" || false,
    };
...
function parseAPIContent(text) {
  try {
    ...
    const fromTimestampMatch = text.match(
      /\[fromtimestamp\]([\s\S]*?)\[\/fromtimestamp\]/
    );
    ...
    const toTimestampMatch = text.match(
      /\[totimestamp\]([\s\S]*?)\[\/totimestamp\]/
    );
    if (toTimestampMatch) {
  ...
  if (missing.length > 0) {
    throw new ValidationError(
      `Missing required fields: ${missing.join(
        ", "
      )}. Please provide them in the format [field]value[/field]`
    );
  }
}
...
var actionGetTokenHoldersCount = {
  ...
  validate: async (_runtime, message) => {
    var _a2;
    if (
      ((_a2 = message.content) == null ? void 0 : _a2.type) !==
      "GET_TOKEN_HOLDERS_COUNT_ANKR"
    ) {
      return true;
    }
    ...
    try {
      ...
      if (!parsedContent.chain || !parsedContent.contract) {
        throw new ValidationError(
          "Blockchain and contract address are required"
        );
      }
  ...
  handler: async (runtime, message, _state, _options = {}, callback) => {
    ...
    try {
      ...
      console.log("Debug - Config validated:", {
        hasWallet: !!config14.ANKR_API_KEY,
        env: config14.ANKR_ENV,
      });
      const wallet = config14.ANKR_API_KEY;
      if (!wallet) {
      ...
      try {
        ...
        let formattedText = `Token Holders Count on ${
          ((_c = parsedContent.chain) == null ? void 0 : _c.toUpperCase()) ||
          "UNKNOWN"
        }:
...
Sync Status: ${result.syncStatus.status} (${result.syncStatus.lag})`;
      ...
      throw new APIError(
        "Failed to execute GET_TOKEN_HOLDERS_COUNT_ANKR action"
      );
    }
  },
};
...
Price: $${price} USD
Contract: ${result.contractAddress.slice(
          0,
          6
        )}...${result.contractAddress.slice(-4)}
...
var actionGetTokenTransfers = {
  ...
  handler: async (runtime, message, _state, _options = {}, callback) => {
    ...
    try {
      ...
      validateRequiredFields(parsedContent, [
        "contract",
        "chain",
        "fromTimestamp",
        "toTimestamp",
      ...
      try {
        const response = await axios3.post(
          endpoint,
          {
            ...
            params: {
              ...
              toTimestamp: parsedContent.toTimestamp,
              pageSize:
                ((_b2 = messageContent.filters) == null
                  ? void 0
                  : _b2.pageSize) || 10,
            },
            id: 1,
        ...
        const result = response.data.result;
        let formattedText = `Token Transfers on ${
          ((_c = parsedContent.chain) == null ? void 0 : _c.toUpperCase()) ||
          "UNKNOWN"
        }:
...
var actionGetAccountBalance = {
  ...
  validate: async (_runtime, message) => {
    ...
    try {
      ...
      if (
        ((_c = content.filters) == null ? void 0 : _c.blockchain) &&
        !Array.isArray(content.filters.blockchain)
      ) {
  ...
  handler: async (runtime, message, _state, _options = {}, callback) => {
    ...
    try {
      ...
      console.log("Debug - Message content details:", {
        ...
        hasFilters: !!(messageContent == null
          ? void 0
          : messageContent.filters),
        textContent: messageContent == null ? void 0 : messageContent.text,
        contentType: typeof (messageContent == null
          ? void 0
          : messageContent.text),
      });
      const config14 = await validateankrConfig(runtime);
      ...
      try {
        ...
        let formattedText = `Here are the balances for wallet ${
...
`;
          if (balance.contractAddress) {
            formattedText += `   Contract: ${balance.contractAddress.slice(
              0,
              6
            )}...${balance.contractAddress.slice(-4)}
`;
          }
...
`;
        ...
        if (callback) {
          logGranular4("Sending success callback with formatted text", {
            formattedText,
          });
          callback({
            text: formattedText,
...
var logGranular5 = (message, data) => {
  if (GRANULAR_LOG5) {
    ...
    console.log(
      `[GetTransactionsByAddress] ${message}`,
      data ? JSON.stringify(data, null, 2) : ""
    );
  }
};
var actionGetTransactionsByAddress = {
  ...
  validate: async (_runtime, message) => {
    ...
    if (
      ((_a2 = message.content) == null ? void 0 : _a2.type) !==
      "GET_TRANSACTIONS_BY_ADDRESS_ANKR"
    ) {
      return true;
    }
    ...
    try {
      ...
      if (
        ((_b2 = content.filters) == null ? void 0 : _b2.pageSize) &&
        (content.filters.pageSize < 1 || content.filters.pageSize > 100)
      ) {
  ...
  handler: async (runtime, message, _state, _options = {}, callback) => {
    ...
    try {
      ...
      try {
        const response = await axios5.post(
          endpoint,
          {
            ...
            params: {
              ...
              address: parsedContent.contract,
              pageSize:
                ((_c = messageContent.filters) == null
                  ? void 0
                  : _c.pageSize) || 5,
              includeLogs:
                ((_d = messageContent.filters) == null
                  ? void 0
                  : _d.includeLogs) || true,
            },
            id: 1,
        ...
        const result = response.data.result;
        let formattedText = `Transactions for ${parsedContent.contract} on ${
          ((_e = parsedContent.chain) == null ? void 0 : _e.toUpperCase()) ||
          "UNKNOWN"
        }:
...
`;
        result.transactions.forEach((tx, index) => {
          const date = new Date(
            Number.parseInt(tx.timestamp, 16) * 1e3
          ).toLocaleString();
          const value = Number.parseInt(tx.value, 16) / 1e18;
          const status = tx.status === "0x1" ? "Success" : "Failed";
          formattedText += `${index + 1}. Transaction
`;
          formattedText += `   Hash: ${tx.hash.slice(0, 6)}...${tx.hash.slice(
            -4
          )}
`;
          formattedText += `   From: ${tx.from.slice(0, 6)}...${tx.from.slice(
            -4
          )}
`;
...
`;
      ...
      throw new APIError(
        "Failed to execute GET_TRANSACTIONS_BY_ADDRESS_ANKR action"
      );
    }
  },
};
  ...
  validate: async (_runtime, message) => {
    ...
    try {
      ...
      if (!parsedContent.chain || !parsedContent.txHash) {
        throw new ValidationError(
          "Blockchain and transaction hash are required"
        );
      }
    ...
    try {
      ...
      try {
        ...
        const timestamp = new Date(
          Number.parseInt(transaction.timestamp, 16) * 1e3
        ).toLocaleString();
        ...
        let formattedText = `Transaction Details on ${
          ((_c = parsedContent.chain) == null ? void 0 : _c.toUpperCase()) ||
          "UNKNOWN"
        }:
...
`;
        formattedText += `Status: ${status}
`;
        formattedText += `From: ${transaction.from.slice(
          0,
          6
        )}...${transaction.from.slice(-4)}
`;
        formattedText += `To: ${transaction.to.slice(
          0,
          6
...
`;
      ...
      throw new APIError(
        "Failed to execute GET_TRANSACTIONS_BY_HASH_ANKR action"
      );
    }
  },
};
...
var actionGetBlockchainStats = {
  ...
  description:
    "Retrieve statistical information about specified blockchain networks.",
  examples: [
    [
      {
        user: "user",
  ...
  validate: async (_runtime, message) => {
    ...
    try {
      const content = message.content;
      if (
        ((_b2 = content.filters) == null ? void 0 : _b2.blockchain) &&
        !Array.isArray(content.filters.blockchain)
      ) {
        throw new ValidationError("Blockchain must be an array");
      }
...
`;
          formattedText += `Total Transactions: ${(
            stat.totalTransactionsCount / 1e9
          ).toFixed(1)}B
`;
          formattedText += `Total Events: ${(
            stat.totalEventsCount / 1e9
          ).toFixed(1)}B
`;
          formattedText += `Block Time: ${(stat.blockTimeMs / 1e3).toFixed(
            1
          )} seconds
`;
          formattedText += `Native Coin Price: $${Number(
            stat.nativeCoinUsdPrice
          ).toFixed(2)}

`;
        }
        if (callback) {
          logGranular7("Sending success callback with formatted text", {
            formattedText,
          });
          callback({
            text: formattedText,
  ...
  handler: async (runtime, message, _state, _options = {}, callback) => {
    ...
    try {
      ...
      const requestParams = {
        blockchain: parsedContent.chain,
        pageSize:
          ((_c = messageContent.filters) == null ? void 0 : _c.pageSize) ?? 5,
      };
      console.log("Debug - API request parameters:", {
        params: requestParams,
        endpoint: ANKR_ENDPOINTS.production.multichain,
      ...
      try {
        ...
        let formattedText = `Here are the top currencies from ${
          parsedContent.chain
            ? parsedContent.chain[0].toUpperCase()
            : "Unknown Chain"
        }:
...
`;
        ...
        for (const currency of currencies) {
          formattedText += [
            ...
            currency.address
              ? `   Contract: ${currency.address.slice(
                  0,
                  6
        ...
        if (callback) {
          logGranular8("Sending success callback with formatted text", {
            formattedText,
          });
          callback({
            text: formattedText,
    ...
    [
      {
        ...
        content: {
          ...
          filters: {
            ...
            pageSize: 5,
            pageToken:
              "eyJsYXN0X2Jsb2NrIjoiMTIzNDU2Nzg4IiwibGFzdF9pbnRlcmFjdGlvbl9pbmRleCI6IjEifQ==",
          },
        },
      },
      {
  ...
  handler: async (runtime, message, _state, _options = {}, callback) => {
    ...
    try {
      ...
      const requestParams = {
        ...
        pageSize:
          ((_a2 = messageContent.filters) == null ? void 0 : _a2.pageSize) ?? 5,
        pageToken:
          (_b2 = messageContent.filters) == null ? void 0 : _b2.pageToken,
      };
      try {
...
var actionGetNFTHolders = {
  ...
  validate: async (_runtime, message) => {
    ...
    try {
      ...
      if (
        ((_c = content.filters) == null ? void 0 : _c.blockchain) &&
        typeof content.filters.blockchain !== "string"
      ) {
  ...
  handler: async (runtime, message, _state, _options = {}, callback) => {
    ...
    try {
      ...
      const requestParams = {
        ...
        contractAddress: parsedContent.contract,
        pageSize:
          ((_c = messageContent.filters) == null ? void 0 : _c.pageSize) || 10,
        pageToken:
          (_d = messageContent.filters) == null ? void 0 : _d.pageToken,
      };
      console.log("Debug - API request parameters:", {
        params: requestParams,
        endpoint,
...
${result.holders
  .map((holderAddress, index) => `${index + 1}. ${holderAddress}`)
  .join("\n")}

${
  result.nextPageToken
    ? "More holders available. Use the page token to see more.\n"
    : ""
}
${
  result.syncStatus
...
var actionGetNFTTransfers = {
  ...
  description:
    "Get NFT transfer history for a specific address or contract on eth.",
  // Fix the example data to match the interface
  examples: [
    [
      {
  ...
  validate: async (_runtime, message) => {
    ...
    try {
      ...
      if (
        !((_b2 = content.filters) == null ? void 0 : _b2.blockchain) ||
        !((_c = content.filters) == null ? void 0 : _c.contractAddress)
      ) {
        throw new ValidationError(
          "Blockchain and contract address are required"
        );
    ...
    try {
      ...
      try {
        ...
        transfers.forEach((transfer, index) => {
          formattedText += `${index + 1}. Transfer of ${transfer.tokenName} (${
...
`;
          formattedText += `   Time: ${new Date(
            transfer.timestamp * 1e3
          ).toLocaleString()}
`;
...
`;
        ...
        if (callback) {
          logGranular11("Sending success callback with formatted text", {
            formattedText,
          });
          callback({
            text: formattedText,
        ...
        content: {
          ...
          data: {
            ...
            attributes: {
              ...
              name: "Bored Ape #1234",
              description:
                "A unique Bored Ape NFT living on the Ethereum blockchain",
              traits: [
  ...
  validate: async (_runtime, message) => {
    ...
    try {
      ...
      if (
        !((_b2 = content.filters) == null ? void 0 : _b2.blockchain) ||
        !((_c = content.filters) == null ? void 0 : _c.contractAddress) ||
        !((_d = content.filters) == null ? void 0 : _d.tokenId)
      ) {
        throw new ValidationError(
          "Blockchain, contract address, and token ID are required"
        );
      }
...
`;
        formattedText += `Collection: ${nftData.attributes.name
          .split("#")[0]
          .trim()}
`;
        formattedText += `Contract: ${nftData.metadata.contractAddress.slice(
          0,
          6
        )}...${nftData.metadata.contractAddress.slice(-4)} (${
          nftData.metadata.contractType
        })
...
`;
        ...
        if (callback) {
          logGranular12("Sending success callback with formatted text", {
            formattedText,
          });
          callback({
            text: formattedText,
...
var actionGetNFTsByOwner = {
  ...
  description:
    "Retrieve all NFTs owned by a specific wallet address across multiple blockchains with detailed metadata.",
  examples: [
    [
      {
        user: "user",
  ...
  validate: async (_runtime, message) => {
    ...
    try {
      ...
      if (
        !((_b2 = content.filters) == null ? void 0 : _b2.blockchain) ||
        !((_c = content.filters) == null ? void 0 : _c.walletAddress)
      ) {
        throw new ValidationError("Blockchain and wallet address are required");
      }
      if (
        ((_d = content.filters) == null ? void 0 : _d.blockchain) &&
        !Array.isArray(content.filters.blockchain)
      ) {
        throw new ValidationError("Blockchain must be an array");
      }
  ...
  handler: async (runtime, message, _state, _options = {}, callback) => {
    ...
    try {
      ...
      const requestParams = {
        ...
        walletAddress: parsedContent.wallet,
        pageSize:
          ((_b2 = messageContent.filters) == null ? void 0 : _b2.pageSize) ??
          10,
        pageToken:
          (_c = messageContent.filters) == null ? void 0 : _c.pageToken,
      };
...
`;
          ...
          formattedText += `   Contract: ${nft.contractAddress.slice(
            0,
            6
          )}...${nft.contractAddress.slice(-4)} (${nft.contractType})
`;
...
`;
        ...
        if (callback) {
          logGranular13("Sending success callback with formatted text", {
            formattedText,
          });
          callback({
            text: formattedText,
  ...
  for (const action of actions) {
    actionTable.push([
      ...
      typeof action.validate === "function"
        ? chalk.green("\u2713")
        : chalk.red("\u2717"),
      ((_a = action.examples) == null ? void 0 : _a.length) > 0
        ? chalk.green("\u2713")
        : chalk.red("\u2717"),
      chalk.gray(
        ((_b = action.similes) == null ? void 0 : _b.join(", ")) || "none"
      ),
    ]);
  }
  console.log(`
...
export { ankrPlugin, index_default as default };
